...
function bindPromo() {
  ...
  if (!Discount.gesehen()) {
    let versuche = 0;
    const spaeter = () => {
      // Nur einmal je Besuch — wer ihn weggeklickt hat, bekommt ihn nicht
      // gleich wieder vorgesetzt. Wer den Code schon hat, sieht ihn hier
      // noch einmal, statt sich zu fragen, wo er geblieben ist.
      if (Discount.gesehen()) return;
