...
function bindPromo() {
  ...
  /* Weggeklickt heißt: für diesen Seitenaufruf erledigt. Der Merker liegt
     bewusst im Arbeitsspeicher und nicht im Browserspeicher — sonst bliebe
     der Gutschein aus, wenn jemand die Seite schließt und in derselben
     Sitzung wieder öffnet. In der Einzeldatei ist ein Seitenaufruf zugleich
     der ganze Besuch; die klassische Website merkt es sich zusätzlich für
     die Sitzung, damit der Dialog nicht auf jeder Unterseite erscheint. */
  let wegGeklickt = false;
  const promoGesehen = () => wegGeklickt || (!isPreview() && Discount.gesehen());
  const promoMerken = () => { wegGeklickt = true; if (!isPreview()) Discount.merken(); };
  ...
  const zeigen = (vonHand) => {
    ...
    requestAnimationFrame(oeffnen);
    /* In einem verborgenen Rahmen ruht requestAnimationFrame. Ohne diese
       zweite Chance bliebe der Dialog auf Dauer durchsichtig stehen. */
    setTimeout(oeffnen, 500);
    sperren();
  };
